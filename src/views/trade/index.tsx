...
export function TradeView() {
  ...
  const [showDisclaimer,setShowDisclaimer]=useState<any>(true);

  useEffect(()=>{
  if(localStorage.getItem('confirm-disc')){
  ...
  const [
    SuperBonds_Rewards_Pool_30_Balance,
    setSuperBonds_Rewards_Pool_30_Balance,
  ] = useState(0);
  const [superBonds_status30, setsuperBonds_status30] =
  ...
  const [
    SuperBonds_Rewards_Pool_90_Balance,
    setSuperBonds_Rewards_Pool_90_Balance,
  ] = useState(0);
  const [superBonds_status90, setsuperBonds_status90] =
    useState<any>("INACTIVE");
  ...
  useEffect(() => {
    ...
    if (data30pool) {
      ...
      setTransaction_fee_SuperB30(
        new BN(data30pool.transaction_fee_SuperB, 10, "le").toNumber()
      );
    ...
    if (data90pool) {
      ...
      setTransaction_fee_SuperB90(
        new BN(data90pool.transaction_fee_SuperB, 10, "le").toNumber()
      );
  ...
  const readPoolData_30 = async () => {
    ...
    const encodeSuperBonds_Rewards_Pool_Account_ADDRESS =
      (await connection.getAccountInfo(
        new PublicKey(decodedPoolDataState.SuperBonds_Rewards_Pool),
        "singleGossip"
      ))!.data;
    const decodeSuperBonds_Rewards_Pool_Account_ADDRESS = AccountLayout.decode(
      encodeSuperBonds_Rewards_Pool_Account_ADDRESS
    );
    let SuperBonds_Rewards_Pool_Balance =
      new BN(
        decodeSuperBonds_Rewards_Pool_Account_ADDRESS.amount,
        10,
        "le"
      ).toNumber() /
      10 ** USDC_DECIMALS;
  ...
  const getPlatformData = async () => {
    ...
    const response: AxiosResponse<any> = await axios.get(
      "https://mainnet-api.superbonds.finance/platformData "
    );
  ...
  const onTrade = async (pool: any) => {
    ...
    let bondValueMaturity =
      pool == 30 ? bondValueMaturity_30 : bondValueMaturity_90;
    let bondYield = pool == 30 ? bond_yield30 : bond_yield90;
    let trade_fee_USDC = pool == 30 ? trade_fee_USDC30 : trade_fee_USDC90;
    let adjustedLiquidity =
      pool == 30 ? adjustedLiquidity30 : adjustedLiquidity90;
    let tradeLiquidityAvailability =
      pool == 30 ? tradeLiquidityAvailability30 : tradeLiquidityAvailability90;
    let lp_token_mint_address =
      pool == 30 ? LP_TOKEN_30_MINT_ADDRESS : LP_TOKEN_90_MINT_ADDRESS;
    ...
    let toSend =
      bondYield > 0
        ? bondValue * (1 + trade_fee_USDC)
        : bondValueMaturity * (1 + trade_fee_USDC);
    let superB_fee =
      pool == 30 ? transaction_fee_SuperB30 : transaction_fee_SuperB90;
    ////console.log('toSend',toSend,'superB_fee',superB_fee,'adjustedLiquidity',adjustedLiquidity);
    if (
      bondValueMaturity - bondValue >
      tradeLiquidityAvailability * adjustedLiquidity
    ) {
      notify({
    ...
    let Fee_Message =
      bondYield > 0
        ? bondValue * trade_fee_USDC
        : bondValueMaturity * trade_fee_USDC;
    // let message =
            ...
            <tr>
              ...
              <td class="text-right">
                <span class="td_span span_green">
                  ${
                    superbondsStatus
                      ? Math.floor(bondYield * superbondsRate * 100) / 100
                      : bondYield
                  }%</span>
              </td>
            </tr>
    ...
    let rentExemption = 0;
    try {
      rentExemption = await connection.getMinimumBalanceForRentExemption(
        MintLayout.span,
        "singleGossip"
      );
      ...
      if (rentExemption == 0) {
        notify({
          message:
            "Please try again, connection to Solana blockchain was interrupted",
          type: "error",
        });
        return;
      }
    ...
    let nft_associated_token_account_address = await findAssociatedTokenAddress(
      publicKey,
      NFT_Mint_account.publicKey
    );
    ////console.log('nft_associated_token_account_address',nft_associated_token_account_address.toBase58());

    let nft_associated_token_account_creationIx =
    ...
    let usdc_associated_token_account_address =
      await findAssociatedTokenAddress(publicKey, USDC_MINT_ADDRESS);
    ...
    let superB_associated_token_account_address =
      await findAssociatedTokenAddress(publicKey, SUPERB_MINT_ADDRESS);
    ...
    const buffers = [
      Buffer.from(
        Uint8Array.of(
          15,
          ...new Numberu64(Math.round(toSend * 10 ** USDC_DECIMALS)).toBuffer(),
          ...new Numberu64(
            Math.round(min_interest * 10 ** USDC_DECIMALS)
          ).toBuffer()
        )
      ),
    ];
    ...
    //return;
    let [SUPERBONDS_REWARD_PDA, SUPERBONDS_REWARD_NONCE] =
      await PublicKey.findProgramAddress(
        [new PublicKey(datapool.SuperBonds_Rewards_Pool).toBuffer()],
        SUPERBONDS_PROGRAM_ID
      );

    const encodedStakingDataState = (await connection.getAccountInfo(
      PLATFORM_DATA_ACCOUNT,
      "singleGossip"
    ))!.data;
    const decodedStakingDataState = PLATFORM_DATA_LAYOUT.decode(
      encodedStakingDataState
    ) as PlatformDataLayout;

    let [SuperB_pda_address, SuperB_pda_NONCE] =
      await PublicKey.findProgramAddress(
        [new PublicKey(decodedStakingDataState.SuperB_Account).toBuffer()],
        SUPERBONDS_PROGRAM_ID
      );
    ...
    if (resp.length == 0) {
      ...
      const TradeIx = new TransactionInstruction({
        ...
        keys: [
          ...
          {
            pubkey: trader_Data_account.publicKey,
            isSigner: false,
            isWritable: true,
          },
          { pubkey: publicKey, isSigner: true, isWritable: false },
          {
            pubkey: new PublicKey(decodedStakingDataState.SuperB_Account),
            isSigner: false,
            isWritable: true,
          },
          {
            pubkey: trade_state_account.publicKey,
            isSigner: false,
            isWritable: true,
          },
          {
            pubkey: usdc_associated_token_account_address,
            isSigner: false,
            isWritable: true,
          },
          {
            pubkey: superB_associated_token_account_address,
            isSigner: false,
            isWritable: true,
          },
          {
            pubkey: nft_associated_token_account_address,
            isSigner: false,
            isWritable: false,
          },
          {
            pubkey: NFT_Mint_account.publicKey,
            isSigner: false,
            isWritable: false,
          },
          {
            pubkey: new PublicKey(datapool.LP_Pool),
            isSigner: false,
            isWritable: true,
          },
          {
            pubkey: new PublicKey(platformData.SuperB_Pool),
            isSigner: false,
            isWritable: true,
          },
          {
            pubkey: new PublicKey(datapool.Traders_Pool),
            isSigner: false,
            isWritable: true,
          },
          {
            pubkey: new PublicKey(datapool.SuperBonds_Rewards_Pool),
            isSigner: false,
            isWritable: true,
          },
          {
            pubkey: new PublicKey(platformData.Treasury),
            isSigner: false,
            isWritable: true,
          },
          ...
          {
            pubkey: new PublicKey(SUPERBONDS_REWARD_PDA.toString()),
            isSigner: false,
            isWritable: false,
          },
      ...
      let txid1 = await sendTransaction(
        connection,
        wallet,
        [
      ...
      if (!txid1) {
        ...
        let txid2 = await sendTransaction(
          connection,
          wallet,
          [TradeIx],
          [trade_state_account, NFT_Mint_account, trader_Data_account]
        ...
        if (!txid2) {
          ...
          const data = {
            NFT: NFT_Mint_account.publicKey.toBase58(),
            type: "add",
          };
          const response: AxiosResponse<any> = await axios.post(
            "https://mainnet-api.superbonds.finance/addTradeRequest",
            data
          );
      ...
      if (!txid1) {
        ...
        let txid2 = await sendTransaction(
          connection,
          wallet,
          [createTradeStateAccountIx, TradeIx],
  ...
  const fetchPublicAPI = async (limit: Number, offset: Number) => {
    ...
    try {
      ...
      const response: AxiosResponse<any> = await axios.post(
        "https://mainnet-api.superbonds.finance/getAllTrades",
        data
      );
  ...
  const fetchTrades = async (limit: Number, offset: Number) => {
    ...
    if (publicKey) {
      try {
        ...
        const response: AxiosResponse<any> = await axios.post(
          "https://mainnet-api.superbonds.finance/getTrades",
          data
        );
        if (response.data.trades.length === 0 && offset > 0) {
          fetchTrades(10, 0);
  ...
  const fetchPendingTrades = async (limit: Number, offset: Number) => {
    ...
    if (publicKey) {
      try {
        ...
        const response: AxiosResponse<any> = await axios.post(
          "https://mainnet-api.superbonds.finance/getPendings",
          data
        );
  ...
  const fetchAPY = async () => {
    const response: AxiosResponse<any> = await axios.get(
      "https://mainnet-api.superbonds.finance/Trade_Staking_APY"
    );
  ...
  useInterval(() => {
    superBondsProcess();
    let publicKey = wallet.publicKey;
    if (!stopFetchingTrades) fetchPublicAPI(10, 0);
  ...
  const onSettle = async (
    ...
    if (pool == POOL_30_ADDRESS) {
      const encodeTraders_Pool_Account_ADDRESS =
        (await connection.getAccountInfo(
          new PublicKey(data30pool.Traders_Pool),
          "singleGossip"
        ))!.data;
      const decodedTraders_Pool_Account = AccountLayout.decode(
        encodeTraders_Pool_Account_ADDRESS
      );
      let Traders_Pool_Balance =
        new BN(decodedTraders_Pool_Account.amount, 10, "le").toNumber() /
        10 ** USDC_DECIMALS;
      ...
      const encodeTraders_Pool_Account_ADDRESS =
        (await connection.getAccountInfo(
          new PublicKey(data90pool.Traders_Pool),
          "singleGossip"
        ))!.data;
      const decodedTraders_Pool_Account = AccountLayout.decode(
        encodeTraders_Pool_Account_ADDRESS
    ...
    const buffers = [Buffer.from(Uint8Array.of(39))];
    let [TRADER_POOL_TOKEN_PDA, TRADER_POOL_TOKEN_NONCE] =
      await PublicKey.findProgramAddress(
        [new PublicKey(decodedPoolDataState.Traders_Pool).toBuffer()],
        SUPERBONDS_PROGRAM_ID
      );
    const settleIx = new TransactionInstruction({
      ...
      keys: [
        ...
        {
          pubkey: new PublicKey(usdc_account),
          isSigner: false,
          isWritable: true,
        },
        {
          pubkey: new PublicKey(TRADER_POOL_TOKEN_PDA.toString()),
          isSigner: false,
          isWritable: false,
        },
        {
          pubkey: new PublicKey(decodedPoolDataState.Traders_Pool),
          isSigner: false,
          isWritable: true,
        },
        { pubkey: TOKEN_PROGRAM_ID, isSigner: false, isWritable: false },
    ...
    let txid = await sendTransaction(connection, wallet, [settleIx], []);
    ...
    if (data30pool) {
      if (data30pool.is_superbonds_time) {
        if (SuperBonds_Rewards_Pool_30_Balance <= 1) {
          ...
          let BV_after_SB =
            bondValue *
            (1 + (data30pool.superBonds_rate * bond_yield30) / 10000) **
              (30 / 365);
          ...
          if (diff > SuperBonds_Rewards_Pool_30_Balance / 10) {
            ...
            let max_BV_at_maturity =
              BV_before_SB + SuperBonds_Rewards_Pool_30_Balance / 10;
            let YpowerX = max_BV_at_maturity / bondValue;
            //ln(y^x) = x ln (y)
            let lny = Math.log(YpowerX) / (30.0 / 365.0);
            //e^lny = y
            superbonds_rate =
              ((Math.pow(Math.E, lny) - 1) * 100) / bond_yield30;
          } else if (diff > 0) {
          ...
          setBondValueMaturity_30(
            bondValue *
              (1 + (superbonds_rate * bond_yield30) / 100) ** (30 / 365)
          );
        }
    ...
    if (data90pool) {
      if (data90pool.is_superbonds_time) {
        if (SuperBonds_Rewards_Pool_90_Balance <= 1) {
          ...
          let BV_after_SB =
            bondValue *
            (1 + (data90pool.superBonds_rate * bond_yield90) / 10000) **
              (90 / 365);
          let diff = BV_after_SB - BV_before_SB;
          let superbonds_rate = 1;
          if (diff > SuperBonds_Rewards_Pool_90_Balance / 10) {
            //find yield at max rate
            let max_BV_at_maturity =
              BV_before_SB + SuperBonds_Rewards_Pool_90_Balance / 10;
            let YpowerX = max_BV_at_maturity / bondValue;
            //ln(y^x) = x ln (y)
            let lny = Math.log(YpowerX) / (90.0 / 365.0);
            //e^lny = y
            superbonds_rate =
              ((Math.pow(Math.E, lny) - 1) * 100) / bond_yield90;
          } else if (diff > 0) {
            superbonds_rate = data90pool.superBonds_rate / 100;
          }
          ////console.log('superbonds_rate',superbonds_rate);
          setSuperBondsRate90(superbonds_rate);
          setBondValueMaturity_90(
            bondValue *
              (1 + (superbonds_rate * bond_yield90) / 100) ** (90 / 365)
          );
        }
  ...
  const handlePagination = (limit: number, x_paginationcursor: number) => {
    ...
    if (x_paginationcursor > 0) {
      ...
      else fetchPendingTrades(limit, offset + x_paginationcursor);
      ...
      setOffset(offset + 0);
      if (showAllTrade === 1) fetchPublicAPI(limit, x_paginationcursor);
      else if (showAllTrade == 2) fetchTrades(limit, x_paginationcursor);
      else fetchPendingTrades(limit, x_paginationcursor);
    }
  };
  ...
  const handleMaxBalance = (pool: number) => {
    ...
    if (pool === 30) {
      let netAmount = formatNumberWithoutRounding.format(
        USDCbalance - USDCbalance * trade_fee_USDC30
      );
      ...
      let netAmount = formatNumberWithoutRounding.format(
        USDCbalance - USDCbalance * trade_fee_USDC90
      );
 ...
 const handleDisclaimerShow=()=>{
  ...
  return (
    <>
   {showDisclaimer && <MyModal  isShow={showDisclaimer} handleShow={handleDisclaimerShow}/>}
        ...
        <div className="w-9/12 my-0 mx-auto pt-20 lg:pt-24 md:pt-20 2xl:w-9/12 lg:w-11/12 xl:w-10/12 min-xxl:w-7/12  max-2xl:w-8/12">
          <div className="flex flex-col">
            ...
            <div className="flex pt-6 justify-center md:flex-wrap">
              <div
                className={`flex w-6/12 md:w-full flex-col bg-gray-300 pb-7 neon-bottom-card selected-box-neon  rounded-md ${
                  superBonds_status30 === "ACTIVE"
                    ? "neon-bottom-card selected-box-neon"
                    : ""
                } ${cardType === "30" ? "sm:block" : "sm:hidden"}`}
              >
                <div
                  className={`flex justify-between rounded-t-md py-3 px-3 ${
                    superBonds_status30 === "ACTIVE"
                      ? "sb-activated"
                      : "bg-gray-200"
                  } select-none`}
                >
                  <Text
                    ...
                    transform={"true"}
                    color={
                      superBonds_status30 === "ACTIVE" ? "#334150" : "white"
                    }
                ...
                <div className="grid-cols-2 mt-3 items-center hidden sm:grid">
                  <div
                    ...
                    onClick={() => setCardType("30")}
                  >
                    <Text
                      weight={"true"}
                      size={"16px"}
                      transform={"true"}
                      color={cardType === "30" ? "#7CFA4C" : "#435160"}
                      style={{ fontFamily: "Archivo" }}
                    >
                      30-day pool
                    </Text>
                    <div
                      className="hidden lg:block md:hidden sm:block w-10/12 m-auto"
                      style={{
                        borderBottom:
                          "3px solid " +
                          (cardType === "30" ? "#7CFA4C" : "#435160"),
                        marginBottom: "6px",
                      }}
                    ></div>
                  </div>
                  <div
                    className="col-span-1 text-center cursor-pointer"
                    onClick={() => setCardType("90")}
                  >
                    <Text
                      weight={"true"}
                      size={"16px"}
                      transform={"true"}
                      color={cardType === "90" ? "#7CFA4C" : "#435160"}
                      style={{ fontFamily: "Archivo" }}
                    >
                      90-day pool
                    </Text>
                ...
                <div className="px-7 sm:px-4">
                  ...
                  <div className="grid grid-cols-10 lg:grid-cols-5 md:grid-cols-4 sm:grid-cols-5 xs:grid-cols-1 xs:grid-rows-2 mt-3 rounded-md items-center">
                    <div className="flex flex-col items-center col-span-3 lg:col-span-2 md:col-span-1 sm:col-span-2 xs:col-span-1 main_hex">
                      <div
                        className="flex flex-col text-center bg-no-repeat bg-center justify-center"
                        style={{
                          wordWrap: "break-word",
                          width: "100%",
                          height: "147px",
                          backgroundSize: "147px",
                          backgroundImage: `url('https://res.cloudinary.com/drr1rnoxf/image/upload/v1649338054/Group_1312_m4dht4.png')`,
                        }}
                      >
                        <Text
                          className="text-grid cursor-pointer w-9/12 mx-auto px-2"
                          size="12px"
                          weight="600"
                          color="white"
                        >
                          ...
                          <Tooltip
                            placement="bottom"
                            title={
                              "Estimated yield earned by FD purchasers, inclusive of a fixed and variable component"
                            }
                          >
                            <ImInfo className="info-circle-hide ml-0.5" />
                          </Tooltip>
                        </Text>
                        <Text
                          className="hex_number"
                          size={CalculateHexNumberSize(
                            (APY + bond_yield30 > 0
                    ...
                    <div
                      className="w-full px-1 py-3 rounded-md col-span-7 lg:col-span-3 md:col-span-3 sm:col-span-3 xs:col-span-1 hex_adjacent"
                      style={{
                        height: "max-content",
                        background:
                          superBonds_status30 === "ACTIVE"
                    ...
                    >
                      <div className="flex justify-evenly lg:flex-col md:flex-row sm:flex-col lg:justify-start md:justify-evenly sm:justify-start">
                        ...
                        >
                          <Text
                            className="text-grid cursor-pointer"
                            size={"14px"}
                            opacity={"0.5"}
                            spacing={"0px"}
                            weight="bold"
                          >
                            ...
                            <Tooltip
                              placement="bottom"
                              title={
                                "Total USDC value of FDs that can be purchased given the USDC value of capital in the LP pool"
                              }
                            >
                        ...
                        ></div>
                        <div
                          className="hidden lg:block md:hidden sm:block"
                          style={{
                            borderBottom:
                              "3px solid " +
                              (superBonds_status30 === "ACTIVE"
                        ...
                        <HoverToolTip
                          noColor
                          className="text-grid flex flex-col"
                        >
                          <Text
                            className="text-grid cursor-pointer"
                            size={"14px"}
                          ...
                          >
                            ...
                            <Tooltip
                              placement="bottom"
                              title={
                                "Amount of USDC that is eligible for payment as interest during yield boost periods"
                              }
                            >
                              <ImInfo className="info-circle ml-0.5" />
                            </Tooltip>
                          ...
                          >
                            <span>
                              <strong>
                                {SuperBonds_Rewards_Pool_30_Balance.toFixed(2)}
                              </strong>
                            </span>
                          </Text>
                          <Text
                  ...
                  <div className="bg-gray-200 py-1 px-1 sm:p-1 mt-5 rounded-md">
                    <div className="w-full p-4 rounded-md bg-gray-400">
                      <HoverToolTip className="text-grid grid grid-cols-3 cursor-pointer">
                        ...
                        >
                          ...
                          <Tooltip
                            placement="rightTop"
                            title={
                              "The guaranteed yield upon maturity, in USDC, expressed in annualized terms"
                            }
                          >
                            {" "}
                            <ImInfo className="info-circle" />
                          </Tooltip>
                        </Text>
                        <Text className="" size={"19px"} color={"#7CFA4C"}>
                          <span
                            style={{
                              color: bond_yield30 >= 0 ? "#7CFA4C" : "red",
                            }}
                          >
                            <strong>
                              {superbondsStatus30
                                ? Math.floor(
                                    bond_yield30 * superbondsRate30 * 100
                                  ) / 100
                                : bond_yield30}
                              %
                            </strong>
                      ...
                      <HoverToolTip className="text-grid grid grid-cols-3 cursor-pointer">
                        ...
                        <Text className="" size={"19px"} color={"white"}>
                          30
                        </Text>
                      </HoverToolTip>
                    </div>
                    <div className="w-full p-4 rounded-md">
                      <HoverToolTip className="text-grid cursor-pointer grid grid-cols-3">
                        ...
                        >
                          ...
                          <Tooltip
                            placement="rightTop"
                            title={
                              "Largest possible trade size at any given point in time"
                            }
                          >
                            <ImInfo className="info-circle ml-0.5" />
                          </Tooltip>
                          ...
                          <Text size="15px">
                            {bond_yield30
                              ? formatNumberWithoutRounding.format(
                                  (adjustedLiquidity30 *
                                    tradeLiquidityAvailability30) /
                                    ((1 + bond_yield30 / 100) ** (30 / 365) - 1)
                                )
                              : "0.00"}
                          </Text>
                          &nbsp;
                          <Text className="whitespace-nowrap" size="12px">
                            USDC
                          </Text>
                        </span>
                      </HoverToolTip>
                      ...
                      <HoverToolTip className="text-grid cursor-pointer grid grid-cols-3">
                        <Text
                          ...
                          opacity={"50%"}
                        >
                          FD Value at Entrance
                          <Tooltip
                            placement="rightTop"
                            title={
                              " Amount paid by the trader and the amount on which the fixed interest is tabulated. Fees not included in calculation"
                            }
                          >
                            <ImInfo className="info-circle ml-0.5" />
                          </Tooltip>
                        ...
                        >
                          <Text size="15px">
                            {formatNumberWithoutRounding.format(bondValue_30)}
                          </Text>
                          &nbsp;
                          <Text className="whitespace-nowrap" size="12px">
                            USDC
                      ...
                      <HoverToolTip className="text-grid cursor-pointer grid grid-cols-3">
                        <Text
                          ...
                          opacity={"50%"}
                        >
                          FD Value at Maturity
                          <Tooltip
                            placement="rightTop"
                            title={
                              "The gross amount (principal + interest) the trader will get back at maturity. Fees not included in calculation"
                            }
                          >
                            <ImInfo className="info-circle ml-0.5" />
                          </Tooltip>
                  ...
                  <div className=" bg-gray-200 py-3 px-3 border rounded-md mt-3 align-middle">
                    <div className="grid grid-cols-2 gap-2">
                      <div className="flex flex-col">
                        <Text size="15px" weight className="" opacity={"0.5"}>
                          Amount
                        </Text>
                        <Text
                          size="12px"
                      ...
                      <div className="">
                        <input
                          ...
                          className="w-full py-2 px-2 h-10 float-right rounded-md bg-gray-400
                                  focus:outline-none ring-2 ring-green-100 border-transparent placeholder-green-100"
                  ...
                  <div>
                    <button
                      className="rounded-sm mt-4 text-center bg-green-100 py-2 w-full  transform transition hover:scale-105"
                      onClick={() => onTrade(30)}
                    >
                      <Text color={"#000000"} weight="true">
                        BUY
                      </Text>
                    </button>
                  </div>
              ...
              <div
                className={`flex w-6/12 md:w-full md:mt-5 flex-col bg-gray-300 pb-7 ml-4 rounded-md md:ml-0 neon-bottom-card selected-box-neon ${
                  superBonds_status90 === "ACTIVE"
                    ? "neon-bottom-card selected-box-neon"
                    : ""
                } ${cardType === "90" ? "sm:block sm:mt-0" : "sm:hidden"}`}
              >
                <div
                  className={`flex justify-between rounded-t-md py-3 px-3 ${
                    superBonds_status90 === "ACTIVE"
                ...
                >
                  <Text
                    ...
                    transform={"true"}
                    color={
                      superBonds_status90 === "ACTIVE" ? "#334150" : "white"
                    }
                    style={{ fontFamily: "Archivo" }}
                  >
                    90-day pool
                ...
                <div className="px-7 sm:px-4">
                  ...
                  <div className="grid grid-cols-10 lg:grid-cols-5 md:grid-cols-4 sm:grid-cols-5 xs:grid-cols-1 xs:grid-rows-2 mt-3 rounded-md items-center">
                    <div className="flex flex-col items-center col-span-3 lg:col-span-2 md:col-span-1 sm:col-span-2 xs:col-span-1 main_hex">
                      ...
                      >
                        <Text
                          className="w-9/12 text-grid cursor-pointer mx-auto px-2"
                          size="12px"
                          weight="600"
                          color="white"
                        >
                      ...
                      <div className="flex justify-evenly lg:flex-col md:flex-row sm:flex-col lg:justify-start md:justify-evenly sm:justify-start">
                        ...
                        >
                          ...
                          >
                            <span>
                              <strong>
                                {SuperBonds_Rewards_Pool_90_Balance.toFixed(2)}
                              </strong>
                            </span>
                          </Text>
                          <Text
                  ...
                  <div className="bg-gray-200 py-1 p-1  sm:p-1 mt-5 rounded-md">
                    <div className="w-full p-4 rounded-md bg-gray-400">
                      <HoverToolTip className="text-grid cursor-pointer grid grid-cols-3">
                        ...
                        <Text weight="true" size={"19px"} color={"#7CFA4C"}>
                          <span
                            style={{
                              color: bond_yield90 >= 0 ? "#7CFA4C" : "red",
                            }}
                          >
                            <strong>
                              {superbondsStatus90
                                ? Math.floor(
                                    bond_yield90 * superbondsRate90 * 100
                                  ) / 100
                                : bond_yield90}
                              %
                            </strong>
                    ...
                    <div className="w-full p-4 rounded-md">
                      ...
                      <HoverToolTip className="text-grid cursor-pointer grid grid-cols-3">
                        ...
                        >
                          <Text size="15px">
                            {formatNumberWithoutRounding.format(bondValue_90)}
                          </Text>
                          &nbsp;
                          <Text className="whitespace-nowrap" size="12px">
                            USDC
                    ...
                    <div className="grid grid-cols-2 ">
                      <div className="flex flex-col">
                        ...
                        <Text
                          ...
                          className="cursor-pointer"
                          opacity={"0.5"}
                          onClick={() => handleMaxBalance(90)}
                        >
                          Balance:{" "}
                          {formatNumberWithoutRounding.format(
                            parseFloat(USDCbalance.toString())
                          )}{" "}
                          <Text className="ml-1" size="12px">
                            USDC
                          </Text>
                        </Text>
                      </div>
          ...
          <div className="mt-2">
            <div className="pb-3 flex justify-between flex-wrap">
              <div className="flex justify-between sm:flex-col sm:justify-start flex-wrap sm:pb-3">
                <Text
                  className={
                    "cursor-pointer ml-2 py-1 " +
                    (showAllTrade == 2 ? "border-b-2 border-green-100" : "")
                  }
                  transform
                  size="14px"
                  onClick={() => onShowAllTrades(2)}
                >
                  MY Trades
                </Text>
                ...
                <Text
                  ...
                  className={
                    "cursor-pointer ml-2 py-1 " +
                    (showAllTrade == 3 ? "border-b-2 border-green-100" : "")
                  }
                  transform
                  size="14px"
                  onClick={() => onShowAllTrades(3)}
                >
                  <Badge
                    offset={[7, 0]}
                    count={
                      myPendingData.length > 9 ? "9+" : myPendingData.length
                    }
                  >
              ...
              <div>
                <div className="bg-gray-300 items-center align-middle py-2 px-5 rounded-md flex">
                  <i
                    onClick={() => handlePagination(10, offset > 0 ? -10 : 0)}
                    className="fas fa-chevron-left mr-4 inline-block cursor-pointer text-green-100 transform transition hover:scale-150"
                  ></i>
                  <i
                    onClick={() => handlePagination(10, 10)}
                    className="fas fa-chevron-right inline-block ml-4 cursor-pointer text-green-100  transform transition hover:scale-150"
                  ></i>
                </div>
              </div>
            </div>
...
const CalculateHexNumberSize = (numberLength: number) => {
  let initialSize = 14;
  if(numberLength > 8){
    initialSize = initialSize - (numberLength - 8)
  }
  if (numberLength > 14) {
    initialSize = 8;
  }
